...
function map2RBF(Map, Nv, dirin=missing, sensor=missing)
# Note: dirin and sensor are needed only for saving the .PNG of the RBF plot.
  #Create an RBF from the given error map.
  println("RBF_GRID: ",Nv)
  ...
  if RBF_PLOT_ON()
    ...
    if typeof(dirin) != Missing  && typeof(sensor) != Missing
