...
using jBlock       #added 2019/6/5 as a means of replacing global block parameters
...
function leasqr!(block::Block,x,y,pin,F,stol,niter,wt,dp,dFdp,options)
...
#   the covariance matrix of the data is assumed to be proportional t,F
...
#
#f=feval(F,x,p);
    f = F(block,x,p)  #applying the function directly in Julia
    fbest=f; pbest=p
r=wt.*(y-f)
ss=r'*r
sbest=ss
...
#println("pbest: ", pbest)
  ...
  #prt=feval(dFdp,x,fbest,pprev,dp,F)
        ...
        prt = dFdp(block,x,fbest,pprev,dp,F)  #for Julia 9/9/2018
            #prt = x,fbest,pprev,dp,fpoly)  #for Julia, test only
...
jac = dFdp(block,x,f,p,dp,F)  #for Julia
msk = dp != 0
...
if b.sensor==2
   ...
   b.sw = p[4]
   b.gv = p[5]
   b.gu = -(sv*gv + sw) #Note: [1 sv sw].[gu gv 1] = |s||g|cos(theta) = 0
   b.flen = p[6]
   ...
   b.sw = p[4]
   b.gu = p[5]
   b.gv = -(su*gu + sw) #Note: [1 sv sw].[gu gv 1] = |s||g|cos(theta) = 0
   b.flen = p[6]
   ccd = [b.sx,b.sy,b.sz] + b.flen*[b.gu,b.gv,b.gw]/norm([b.gu,b.gv,b.gw])
   b.ccdx = ccd[1]
   b.ccdy = ccd[2]
...
#Create a new block to be used in place of global parameters.
bfa = missing
block = Block(sx,sy,sz,su,sv,sw,gu,gv,gw,ccdx,ccdy,ccdz,ccdu,ccdv,ccdw,flen,x0,tglass,nglass,sensor,bfa)
